...
No figure is pre-rendered; every one is generated from the data and written to `figures/`.
...
├── figures/                        # the eight exported figures, numbered
├── requirements.txt
└── .gitignore
```
...
- Situational entropy (Shannon binary entropy of pass rate within cells matched on down, distance,
  ...
  for game state - 79.4% of plays fall in a cell of ten or more
- Formation and game half folded into the conditioning set from the outset rather than held back
  as style, on the evidence that formation splits pass rate more sharply than down does; wider
  distance and score bands pay for the two extra dimensions
- A specification curve over the conditioning set, showing which findings survive rebanding and
  which do not
